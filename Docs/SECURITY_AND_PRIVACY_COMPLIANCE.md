...
| **CWE-259 / CWE-798** | Hardcoded Credentials & Secrets | Static regex scan executed across all source files, configurations, and documentation. Zero plaintext keys (`tskey-auth-...`) or hardcoded passwords exist. Tokens are masked dynamically via `re.sub(r'tskey-auth-\S+', 'tskey-auth-[REDACTED]', text)` in `src/utils/logger.py`. | **PASSED (Zero Risk)** |
| **CWE-312 / CWE-313** | Cleartext Storage of Sensitive Data | Sensitive pre-auth keys and bearer tokens are never saved to `profiles.json`. Secrets are exclusively delegated to OS hardware-backed vaults (`keyring`: Windows DPAPI, macOS Keychain, Linux SecretService) via `src/utils/crypto.py`. | **PASSED (Zero Risk)** |
